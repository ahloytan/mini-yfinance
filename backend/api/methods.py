...
def get_income_statement_data(stock):
    ...
    if response.status_code == 200:
        ...
        hasCurrency = data[index_of_property_in_json(data, keys[0])][keys[0]][0]['currencyCode']
        ...
        total_revenue = {
            ...
            for entry in data[index_of_property_in_json(data, keys[0])][keys[0]]
        }
        net_income_from_continuing_operations_ttm = conversion(data[index_of_property_in_json(data, keys[1])][keys[1]][0]['reportedValue']['raw'], exchange_rate)
        income_statement_ttm = conversion(data[index_of_property_in_json(data, keys[2])][keys[2]][0]['reportedValue']['raw'], exchange_rate)
        net_income_from_operating_continuing_operations = {
            entry['asOfDate']: conversion(entry['reportedValue']['raw'], exchange_rate)
            for entry in data[index_of_property_in_json(data, keys[3])][keys[3]]
        }

    return total_revenue, net_income_from_continuing_operations_ttm, income_statement_ttm, net_income_from_operating_continuing_operations, exchange_rate
...
def get_cash_flow_data(stock, exchange_rate):
    ...
    if response.status_code == 200:
        data = response.json()['timeseries']['result']
        free_cash_flow_ttm = conversion(data[index_of_property_in_json(data, keys[0])][keys[0]][0]['reportedValue']['raw'], exchange_rate)
        free_cash_flow = {
            entry['asOfDate']: conversion(entry['reportedValue']['raw'], exchange_rate)
            for entry in data[index_of_property_in_json(data, keys[1])][keys[1]]
        }
        operating_cash_flow = {
            entry['asOfDate']: conversion(entry['reportedValue']['raw'], exchange_rate)
            for entry in data[index_of_property_in_json(data, keys[2])][keys[2]]
        }
        operating_cash_flow_ttm = conversion(data[index_of_property_in_json(data, keys[3])][keys[3]][0]['reportedValue']['raw'], exchange_rate)
